...
const flowMod = require('../../src/electron/ads-create-flow.cjs') as {
  createCreateFlow: (deps: {
    ...
    fingerprint: { getOsFamily?: (k: string) => unknown; getTemplate?: (k: string) => unknown; buildFingerprintConfig: (t: unknown) => { ok: boolean; fingerprintConfig?: unknown; violations: string[] } };
    nowImpl?: () => number;
  }) => {
...
test('happy path: 用真指纹引擎构造、remark 编码意图/OS/机器、proxy=no_proxy、只标未验证', async () => {
  ...
  const r = await flow.createEnvironment({ osFamilyKey: 'windows', intendedAccountLabel: 'A', machineLabel: 'mac-01', groupId: 'g1' });
  ...
  assert.equal(body.fingerprintConfig.device_memory, undefined, '不固定内存，交给 AdsPower 生成');
  assert.equal(body.fingerprintConfig.hardware_concurrency, undefined, '不固定 CPU 核数，交给 AdsPower 生成');
  assert.equal(body.fingerprintConfig.screen_resolution, undefined, '不固定分辨率，交给 AdsPower 生成');
  assert.equal(body.fingerprintConfig.webgl_config, undefined, '不固定 renderer，交给 AdsPower 生成');
  const meta = parseRemark(body.remark);
  assert.ok(meta);
  assert.equal(meta!.intendedAccountLabel, 'A');
  assert.equal(meta!.template, 'windows');
...
test('显式传 name（FB 导入路径）→ 返回体原样带回该名（edge-env-name-live-sync）', async () => {
  ...
  const r = await flow.createEnvironment({ osFamilyKey: 'windows', groupId: 'g1', name: 'Facebook import 1' });
  assert.equal(r.ok, true);
...
test('未知操作系统 → 诚实拒建，不调 createProfile', async () => {
  const w = recordingWriteApi();
  const flow = createCreateFlow({ writeApi: w, fingerprint: realFingerprint });
  const r = await flow.createEnvironment({ osFamilyKey: 'no-such', groupId: 'g1' });
  assert.equal(r.ok, false);
  assert.equal(r.status, 'rejected');
  assert.match(String(r.error), /未知操作系统/);
  assert.equal(w.calls.length, 0);
});
...
test('缺 groupId → 拒建', async () => {
  const w = recordingWriteApi();
  const flow = createCreateFlow({ writeApi: w, fingerprint: realFingerprint });
  const r = await flow.createEnvironment({ osFamilyKey: 'windows' });
  assert.equal(r.ok, false);
  assert.equal(w.calls.length, 0);
});
...
test('护栏/断言未过 → 拒建并带 violations（stub 指纹）', async () => {
  const w = recordingWriteApi();
  const stubFp = {
    getOsFamily: () => ({ key: 'macos', os: 'macos' }),
  ...
  const r = await flow.createEnvironment({ osFamilyKey: 'x', groupId: 'g1' });
  assert.equal(r.ok, false);
  assert.equal(r.status, 'rejected');
...
test('写失败 → 传播 ok:false，status 不为 ready', async () => {
  ...
  const flow = createCreateFlow({ writeApi: w, fingerprint: realFingerprint });
  const r = await flow.createEnvironment({ osFamilyKey: 'windows', groupId: 'g1' });
  assert.equal(r.ok, false);
...
test('单飞互斥：创建在途时重入返回「进行中」，不双建（H5）', async () => {
  ...
  const p1 = flow.createEnvironment({ osFamilyKey: 'windows', groupId: 'g1' }); // 在途
  const r2 = await flow.createEnvironment({ osFamilyKey: 'windows', groupId: 'g1' }); // 重入
...
test('parseRemark: 往返 + 非本 change 备注返回 null', () => {
  const enc = encodeRemark({ intendedAccountLabel: 'B', osFamily: 'macos', machine: 'm2', createdAt: 1 });
  const dec = parseRemark(enc);
  assert.equal(dec!.intendedAccountLabel, 'B');
  assert.equal(dec!.template, 'macos');
...
test('创建时选平台 → remark 带 plat、回执带 platform', async () => {
  const w = recordingWriteApi({ ok: true, userId: 'u-fb' });
  const flow = createCreateFlow({ writeApi: w, fingerprint: realFingerprint });
  const r = (await flow.createEnvironment({ osFamilyKey: 'windows', groupId: 'g1', platform: 'facebook' } as any)) as any;
  assert.equal(r.ok, true);
...
test('创建视频号环境 → remark 与回执保留规范平台值 wechat_channels', async () => {
  ...
  const flow = createCreateFlow({ writeApi: w, fingerprint: realFingerprint });
  const r = (await flow.createEnvironment({ osFamilyKey: 'windows', groupId: 'g1', platform: 'wechat_channels' } as any)) as any;
  assert.equal(r.ok, true);
...
test('创建不传平台 → 回落 xiaohongshu（零回归）', async () => {
  ...
  const flow = createCreateFlow({ writeApi: w, fingerprint: realFingerprint });
  const r = (await flow.createEnvironment({ osFamilyKey: 'windows', groupId: 'g1' })) as any;
