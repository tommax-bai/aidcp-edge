...
test('edge-client: hello carries platform metadata without changing message type', async () => {
  ...
  assert.deepEqual(sent.payload, {
    ...
    platform: 'xiaohongshu',
    app: 'xhs',
    capabilities: ['locating', 'cdp', 'like', 'browse', 'captcha_assist_text_v1', 'client_core_browser_executor_v1', 'client_data_plane_automation_engine_v1'],
  });
...
test('edge-client: hello carries optional account nickname for display enrichment', async () => {
  ...
  assert.deepEqual(sent.payload, {
    ...
    platform: 'facebook',
    app: 'fb',
    capabilities: ['identity', 'overlay', 'comment', 'captcha_assist_text_v1', 'client_core_browser_executor_v1', 'client_data_plane_automation_engine_v1'],
    accountId: '1234567890',
    accountNickname: 'Test User',
  });
