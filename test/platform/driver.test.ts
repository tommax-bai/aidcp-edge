...
test('selectPlatformDriver: unset platform defaults to xiaohongshu driver', () => {
  ...
  assert.deepEqual(driver.edgeCapabilities, [
    'locating',
    'cdp',
    'like',
    'browse',
    'identity_read_self_profile_v1',
  ]);
});
...
test('selectPlatformDriver: wechat_channels registers API-only interaction capabilities', () => {
  ...
  assert.equal(driver.edgeCapabilities.includes('interaction_browser_control_v1'), true);
