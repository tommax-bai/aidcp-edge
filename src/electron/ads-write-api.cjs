...
//    `user/delete` 在 Edge 只允许本机运维逐个确认。管理后台远程删除已改由 Cloud 直调自己的
//    服务端 AdsPower API，Edge 不再 poll/claim/receipt，也不允许批量 envKey；ads-create-flow 内无删除。
...
// user/delete 允许，但 Edge 调用方 MUST 来自本机逐个确认（见头注）。
