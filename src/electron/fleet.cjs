...
const ENV_KEYS_MUST_DROP = [
  ...
  'AIDCP_ADS_PROXY_OVERRIDE', // 旧版启动参数覆盖已退役；仍剔除外部残留，避免重新形成双代理权威
  'AIDCP_ADS_PROXY_AUTHORITY_FD', // 只允许主进程为当前子进程注入匿名 pipe fd
  'AIDCP_ADS_API_BROKER', // 只允许主进程在已建立私有 IPC 的 managed child 上注入
