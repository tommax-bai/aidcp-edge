...
use crate::protocol::{
    CancelRecord, CommandRecord, EffectPhase, NativeCommand, SessionCloseRecord, SessionOpenRecord,
    SessionStatusRecord,
};
use serde::Serialize;
...
impl Engine {
    ...
    pub async fn execute(
        ...
        let deadline_budget = request.deadline_unix_ms.saturating_sub(now_ms);
        let operation_timeout = Duration::from_millis(deadline_budget.min(session.timeout_ms));
        session.active_command_id = Some(request.command_id);
        let outcome = match &request.command {
            NativeCommand::PageProbe(_) => {
                match tokio::time::timeout(operation_timeout, session.cdp.probe_page()).await {
                    Ok(Ok(result)) => {
                        StoredCommandResult::confirmed(CommandOutput::PageProbe(result))
                    }
